...
| **10** | **Projeto 1 - HashSimPy** <br> - Implementação de uma Tabela Hash (Hash Table / Mapa Hash) em Python. <br> - Desenvolvimento de uma classe que mapeia chaves a valores usando uma função de hash e trata colisões através da técnica de Encadeamento Separado (Separate Chaining), utilizando listas ligadas como estrutura secundária.	| Comunicar de forma clara o problema, a solução baseada em MinHashing, a implementação em Python, os resultados da verificação de similaridade e os desafios/aprendizados durante as 5 semanas de desenvolvimento. Demonstrar a ferramenta funcionando.	| Apresentação Oral, Slides, Demonstração de Software (CLI), Síntese de Informação. | [Orientações](Projeto1.pdf) |
